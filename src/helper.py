from multiprocess import *
import os
from optparse import OptionParser
from multiprocessing import Pool


class extracter:

    def extract_ip(buf: bytes) -> list[str]:
        """从bytes流中导出ip的接口。会调用extract_str导出字符串后再挨个查。
        如果是需要考虑性能的场景，建议不用此接口。
        :param buf: 目标字节流
        :return: 包含ip列表的list
        """
        str_list = extracter.extract_str(buf, target="ip")
        ip_list = extracter.ip_search(str_list)
        return ip_list

    def extract_domain(buf: bytes) -> list[str]:
        """从bytes流中导出域名的接口。会调用extract_str导出字符串后再挨个查。
        如果是需要考虑性能的场景，建议不用此接口。
        :param buf: 目标字节流
        :return: 包含domain列表的list
        """
        str_list = extracter.extract_str(buf, target="ip")  # 先做提取再做扫描
        domain_list = extracter.domain_search(str_list)
        return domain_list

    def extract_str(buf: bytes, target="str", mode="strong") -> list[str]:
        """旨在从任意二进制映像中提取字符串。默认进行ip提取，
        边遍历边进行字符的提取，该函数**只提取可见ascii码**。
        即byte大小>= 0x20 and <= 0x7f 的字符。

        如果是需要考虑性能的场景，建议不使用此接口。

        Args: buf -> 目标二进制映像
            target -> 需要提取的字符串类型，目前仅可提取ip和str。
            这个选项具体影响的其实是字符串的长度，str至少提取2位，ip至少提取6位。
            mode -> 该模式决定是否吃掉\x00字符。部分字符串(如C#)存储时，可能是带着\x00。

        Return: 提取的字符串列表

        """
        str_list = []
        ptr = 0  # 读取指针

        while ptr < len(buf):  # 整体循环，当字符串提取循环退出后会在这里重置tmp变量
            str_tmp = ""
            if buf[ptr] >= 0x20 and buf[ptr] < 0x7f:  # 进入字符串提取循环
                while buf[ptr] >= 0x20 and buf[ptr] < 0x7f:
                    str_tmp += chr(buf[ptr])  # 确认是字符，添加
                    ptr += 1
                    if ptr >= len(buf):
                        break
                    if ptr == len(buf):  # 历史遗留
                        # str_list.append(str_tmp)
                        break
                    if mode == "strong" and buf[ptr] == 0:  # Mode
                        ptr += 1  # 这里不是很完整。在unicode模式下，ascii以word字长存储，低位是ascii，高位是0。这里应该对长度做限制。
                        if ptr >= len(buf):  # 防止指针过长，可优化
                            break
                        continue
                if target == "ip":  # target
                    if len(str_tmp) > 6:  # 为ip筛选长度合适的字符串：0.0.0.0
                        str_list.append(str_tmp)
                elif target == "str":  # 去除二进制流中可能被解释为ascii的字符，但这个方法其实比较朴素。
                    if len(str_tmp) > 3:
                        str_list.append(str_tmp)
            else:
                ptr += 1

        return str_list

    def extract_code_str(buf: bytes) -> list[str]:
        """旨在提取代码文件中的字符串信息（用引号包裹的字符串）
        尚不能匹配中文。

        如果是需要考虑性能的场景，建议不使用此接口。
        :param buf: 目标字节流
        :return: 包含代码的字符串列表的list
        """
        # init
        ptr = 0
        str_prefix = 0
        code_strs = []
        code_str = ""
        # 搜索循环
        while ptr < len(buf):
            if str_prefix != 0:
                "找到字符串头后，匹配字符串"
                if buf[ptr] == str_prefix:  # 需要字符串尾匹配字符串头，"",''
                    "找到字符串尾，退出"
                    code_strs.append(code_str)
                    # clear
                    str_prefix = 0
                    code_str = ""
                    ptr += 1
                    continue
                if buf[ptr] >= 0x20 and buf[ptr] < 0x7f:
                    "匹配字符"
                    code_str += chr(buf[ptr])
                    ptr += 1

                    continue
                else:
                    "非明文字符串"
                    str_prefix = 0
                    code_str = ""
                    ptr += 1
                    continue
            if buf[ptr] == ord("\"") or buf[ptr] == ord("\'"):
                "找到字符串头"
                str_prefix = buf[ptr]  # 保存字符串头
                ptr += 1
                continue
            ptr += 1

        return code_strs

    def ip_search(str_list: list) -> list[str]:
        """在字符串列表中使用正则匹配ip。

        Args: str_list->字符串列表源

        Return: 搜索到的ip列表

        """

        pattern = r"((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}"
        # pattern = r'(?<![\.\d])(([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])\.){3}([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])(?![\.\d])'
        # 匹配 0-255的表达式书写方法
        # pattern = r'([1-9]?\d|1\d\d|2[0-4]\d|25[0-5])'

        import re
        ip_list = []
        for s in str_list:
            ip = re.search(pattern, s)
            # 对单字符串递进搜索循环
            while ip != None:  # 如go的二进制文件中可能会出现一个极长字符串的情况，因此做了一条字符串的遍历多次查询
                match_ip = s[ip.start():ip.end()]  # 取到匹配的ip
                ip_list.append(match_ip)
                s = s[ip.end()::]  # 搜索的目标串取到下一位
                ip = re.search(pattern, s)

        return ip_list

    def domain_search(str_list: list) -> list[str]:
        """在字符串列表中使用正则匹配域名，并使用tld库进行一次顶级域名校验。

        Args: str_list->字符串列表源

        Return: 目的域名列表

        """
        import tld  # 域名验证库
        import re
        pattern = r'([a-zA-Z0-9]([a-zA-Z0-9-_]{0,61}[a-zA-Z0-9])?\.){2,4}[a-zA-Z]{2,11}'
        # r'(?=^.{3,255}$)[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+$'
        domain_list = []

        for s in str_list:
            domain = re.search(pattern, s)
            while domain != None:  # 同ip search，做循环取
                match_domain = s[domain.start():domain.end()]
                # 顶级域名验证
                if tld.get_tld(match_domain, fail_silently=True, fix_protocol=True) != None:
                    domain_list.append(match_domain)

                s = s[domain.end()::]
                domain = re.search(pattern, s)

        return domain_list

    def remove_same_in_list(lst: list):
        new_lst = []
        for i in lst:
            for l in i:
                if l not in new_lst:
                    new_lst.append(l)
        return new_lst


class scanner:
    key_list = ["passwd", "password", "secret",
                "key:\"", "key:\'", "key :\"", "key :\'",
                "key=\"", "key=\'", "key =\"", "key =\'",
                "key\'", "key\""
                "token"]

    def __init__(self, buf: bytes = b"", path: str = ""):
        self.buf = buf  # 扫描目标字节流
        self.path = path  # 扫描目标路径
        self.str_list = []  # 提取的字符串
        self.code_str_list = []  # 提取的代码字符串
        self.len_prefix = 0  # 关键词向前提取长度
        self.len_suffix = 25  # 关键词向后提取长度
        self.batch_file_count = 500  # 多进程执行中，一个进程扫描的文件数量
        self.batch_process_count = 10  # 多进程执行中，一次执行的进程数目
        pass

    def search_keyword_sentence(self) -> list[str]:
        """会把字符串中与关键词匹配的部分的一条句子圈起来，具体圈起来的范围
        和len_prefix,len_suffix有关。

        :return: 和关键词有关的句子
        """
        sentences = [""]
        # 遍历字符串列表
        for s in self.str_list:
            tmp = s
            # 遍历关键词列表，找字符串中匹配关键词的部分
            for k in self.key_list:
                # 递进寻找循环 同helper的ip循环思路
                while tmp.find(k) != -1:
                    index = tmp.find(k)
                    # 算一下关键词前后额外包裹的距离，防止越界
                    prefix = self.len_prefix if index > self.len_prefix else index
                    # 如果关键词在字符串中的位置距离头的距离容得下len_prefix，则从关键词前len_prefix的位置开始取
                    suffix = self.len_suffix if len(tmp) - index - \
                        len(k) > self.len_suffix else len(tmp) - index - len(k)
                    # 和prefix一样，只不过考虑到了关键词的长度，考虑关键词尾到字符串尾的长度。
                    # 防止一些奇怪的错误
                    if sentences[-1] == tmp:
                        break
                    # 根据prefix和suffix添加关键词相关的句子
                    sentences.append(
                        tmp[index-prefix:index+len(k)+suffix])
                    tmp = tmp[index+len(k)+suffix::]

        return sentences
        pass

    def search_in_file_list(self, file_list: list[str]):
        keyword_list = []
        ip_list = []
        domain_list = []
        # print(file_list)
        for file in file_list:
            f = open(file, "rb")
            # print(file)
            # if os.stat(file).st_size > 1 * 1024 * 1024:
            #    continue

            buf = f.read()
            f.close()
            # if len(buf) > 4*1024*1024:
            #    continue
            # s0 = extract_str(buf)

            # print(os.path.join(filepath, filename))
            # s1 = keyword_search(key_list, extract_code_str(buf))
            s1 = self.keyword_search2(file)
            s2 = extracter.extract_ip(buf)
            s3 = extracter.extract_domain(buf)
            keyword_list.append(s1)
            ip_list.append(s2)
            domain_list.append(s3)
        return [keyword_list, ip_list, domain_list]
        pass

    def search_in_document_by_multi(self, path: str):
        f_str = open("output3.txt", "w")
        file_list = []
        keyword_list = []
        ip_list = []
        domain_list = []
        for filepath, dirnames, filenames in os.walk(path):
            for filename in filenames:
                tmp = os.path.join(filepath, filename)
                if ".git" in tmp:
                    break
                # if os.stat(tmp).st_size > 1 * 1024*1024:
                #    continue
                file_list.append(tmp)
        process_count = len(file_list) // 500
        if process_count == 0:
            process_count = 1
        # print(process_count)
        for p in range(0, process_count, 10):

            pool = Pool(processes=10)
            for i in range(10):
                print(p+i)
                # print(list(file_list[(p+i)*1000:(p+i+1)*1000]))
                if p + i >= process_count:
                    # print((p+i)*1000, len(file_list))
                    tmp = pool.apply_async(
                        extracter.search_in_file_list, (file_list[(p+i)*500:],)).get()
                else:
                    # print((p+i+1)*1000, len(file_list))
                    tmp = pool.apply_async(
                        extracter.search_in_file_list, (file_list[(p+i)*500:(p+i+1)*500],)).get()

                keywords = tmp[0]
                ips = tmp[1]
                domains = tmp[2]
                keyword_list += (keywords)
                ip_list += (ips)
                domain_list += (domains)
                if p + i >= process_count:
                    break
            pool.close()
            pool.join()
        keyword_list = extracter.remove_same_in_list(keyword_list)
        ip_list = extracter.remove_same_in_list(ip_list)
        domain_list = extracter.remove_same_in_list(domain_list)
        # keyword_list.sort()
        # ip_list.sort()
        # domain_list.sort()
        f_str.write("-----keyword------\r")
        for s in keyword_list:
            # for i in s:
            if len(s) != 0:
                if (":" in s or "=" in s) and s[-1] != ":" and s[-1] != "{":
                    f_str.write(s+"\r")
        # f_str.write("--------ip--------\r")
        # for s in ip_list:
        #     if len(s) != 0:
        #         f_str.write(s+"\r")
        # f_str.write("------domain------\r")
        # for s in domain_list:
        #     if len(s) != 0:
        #         f_str.write(s+"\r")
        f_str.close()
        pass
        pass

    def __search_keyword_sentence(self, path: str) -> list[str]:
        """(过时的)会把字符串中与关键词匹配的部分的一条句子圈起来

        :param path: _description_
        :return: _description_
        """
        about_keyword_list = [""]
        f = open(path, "rb")
        buf = f.read()
        f.close()
        str_list = extracter.extract_str(buf)
        for s in str_list:
            # print(s)
            tmp = s
            count = 0
            for k in self.key_list:
                # print(f"k:{k}")
                while tmp.find(k) != -1:
                    index = tmp.find(k)
                    # print(f"index:{index}")
                    prefix = 25 if index > 25 else index
                    suffix = 25 if len(tmp) - index - \
                        len(k) > 25 else len(tmp) - index - len(k)
                    # if len(about_keyword_list) > 0:
                    if about_keyword_list[-1] == tmp:
                        break
                    about_keyword_list.append(
                        tmp[index-prefix:index+len(k)+suffix])
                    tmp = tmp[index+len(k)+suffix::]
                    # print(f"tmp:{tmp}")
                    # print(
                    #    f"tmp:{tmp}\r\nlen:{len(tmp)}\r\nindex2:{index+len(k)+suffix}")

        return about_keyword_list

    def __search_keyword(self, keywords: list, strs: list) -> list[str]:
        keyword_list = []
        check = 0
        for s in strs:
            for k in keywords:
                if check == 1:
                    check = 0
                    keyword_list.append(s)
                if k in s:
                    keyword_list.append(s)
                    check = 1
                    break
        return keyword_list

    def __search_document(self, path: str):
        f_str = open("output2.txt", "w")
        keyword_list = []
        ip_list = []
        domain_list = []
        for filepath, dirnames, filenames in os.walk(path):
            for filename in filenames:
                tmp = os.path.join(filepath, filename)
                if ".git" in tmp:
                    break
                # if os.stat(tmp).st_size > 1 * 1024*1024:
                #    continue
                # print(tmp)
                f = open(tmp, "rb")
                buf = f.read()
                f.close()

                s0 = extracter.extract_str(buf)

                # print(os.path.join(filepath, filename))
                s1 = self.__search_keyword(
                    self.key_list, extracter.extract_code_str(buf))
                s2 = extracter.extract_ip(buf)
                s3 = extracter.extract_domain(buf)
                keyword_list.append(s1)
                ip_list.append(s2)
                domain_list.append(s3)

        f_str.write("-----keyword------\r")
        for s in keyword_list:
            for i in s:
                f_str.write(i+"\r")
        f_str.write("--------ip--------\r")
        for s in ip_list:
            for i in s:
                f_str.write(i+"\r")
        f_str.write("------domain------\r")
        for s in domain_list:
            for i in s:
                f_str.write(i+"\r")
        f_str.close()
        pass
